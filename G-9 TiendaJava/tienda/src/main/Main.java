...
public class Main {
    ...
    private static Server server = new Server(iplocal, portlocal, ipmonitor, puertomonitor);
    ...
    private static void inicializaTiendas() {
        ...
        int n = 3;
        for (int i = 0; i < n; i++) {
            SendPost.enviaMensajeInicializacion(new Agente(iplocal, portlocal, rollocal, 0), new Agente(ipmonitor, puertomonitor, rolmonitor, idmonitor), "inicializa");
        }
    }
        ...
        while (salir) {
            ...
            if (ficheros != null) {
                for (int x = 0; x < ficheros.length; x++) {
                    recibeArchivo("xmlfiles\\"+ficheros[x]);
                    System.out.println("**********************************");
                }
            }else{
                salir=false;
    ...
    private static void recibeArchivo(String file) {
        try {
            ...
            switch (tipo) {
                case "reg_tienda":
                    System.out.println("reg tienda");
                    server.recibeInicializacion(s.getReceptor().getId(), s.getListaProductos());
                    break;
                case "entrada_tienda":
                    System.out.println("entrada tienda");
                    server.entraCliente(s.getEmisor().getId(), s.getEmisor().getIp(), s.getEmisor().getPuerto(), s.getListaProductos(), s.getReceptor().getId());
                    break;
                case "solicitar_tiendas":
                    System.out.println("solicita tiendas");
                    server.clientePideTiendas(s.getEmisor().getId(), s.getEmisor().getIp(), s.getEmisor().getPuerto(), s.getListaTiendas(), s.getReceptor().getId());
                    break;
                case "salida_tienda":
                    System.out.println("salida tiendas");
                    server.saleCliente(s.getEmisor().getId(), s.getEmisor().getIp(), s.getEmisor().getPuerto(), s.getReceptor().getId());
                    break;
            }
            archivo.delete();
